...
namespace Capa_Diseño
{
    ...
    {
        private static List<TBL_UNIDAD_MEDIDA> Mi_REG = new List<TBL_UNIDAD_MEDIDA>();
        ...
        // Llenar Listado
        private void Llenar_Lista(ref ListView xContenedor, List<TBL_UNIDAD_MEDIDA> nDatos)
        {
            ...
            if (nDatos != null)
            {
                ...
                foreach (TBL_UNIDAD_MEDIDA xREG in nDatos)
                {
        ...
        private bool Validaciones()
        {
            ...
            {
                this.ep_Error.SetError(this.Txt_Descripcion, "Debe de Digitar el Nombre de la UNIDAD DE MEDIDA...");
                yResult = false;
            }
        ...
        private void Frm_Unidad_Medida_VisibleChanged(object sender, EventArgs e)
        {
            if (Program.Bool_Entrar == false)
            { Program.xAcceso = 0; this.Close(); }
        }

        private void Frm_Unidad_Medida_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void Cmd_Cancelar_Click(object sender, EventArgs e)
        {
            ...
            Frm_Unidad_Medida_Load(sender, e);
        }
        ...
        private void Cmd_Guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<cUNIDAD_MEDIDA> Mis_Datos = new List<cUNIDAD_MEDIDA>();
                cUNIDAD_MEDIDA xDatos = new cUNIDAD_MEDIDA();
                ...
                Program.xMsg = string.Empty;
                Program.xMsg = Program.xMETODOS.Insertar_Actualizar_Unidad_Medida(Dt_DATOS);
                if (Program.xMsg.Length > 0)
                {
                    MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Program.Bool_Entrar = false; return;
        ...
        private void Txt_Filtro_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            try
            {
                if (e.KeyChar == (char)Keys.Enter)
                {
                    ...
                    Program.xMsg = string.Empty;
                    Mi_REG = Program.xMETODOS.Listar_Unidad_Medida(out Program.xMsg, this.Txt_Filtro.Text);
                    if (Program.xMsg.Length > 0)
                    {
                        MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
        ...
        private void Lv_Datos_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            if (this.Lv_Datos.Items.Count > 0)
            {
                if (e.IsSelected == true)
                {
                    ...
                    if (Program.Val_Ban == 1)
                    {
                        Frm_Entrada.xID_UM = double.Parse(this.Txt_Codigo.Text).ToString();
                        Frm_Entrada.xDESC_UM = this.Txt_Descripcion.Text;

                        Frm_Factura.xID_UM = double.Parse(this.Txt_Codigo.Text).ToString();
                        Frm_Factura.xDESC_UM = this.Txt_Descripcion.Text;
