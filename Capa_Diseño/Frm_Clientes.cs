...
namespace Capa_Diseño
{
    ...
    {
        private static List<TBL_CLIENTES> Mi_REG = new List<TBL_CLIENTES>();
        ...
        // Llenar Listado
        private void Llenar_Lista(ref ListView xContenedor, List<TBL_CLIENTES> nDatos)
        {
            ...
            xContenedor.Columns.Add("NEGOCIO", 0, HorizontalAlignment.Left);
            ...
            xContenedor.Columns.Add("TIPO", 120, HorizontalAlignment.Left);
        ...
        {            
            ...
            try
            {
                ...
                this.Txt_Negocio.ReadOnly = true;
                ...
                Llenar_Lista(ref this.Lv_Clientes, Mi_REG);

                if (this.Lv_Clientes.Items.Count > 0)
                {
                    this.Lbl_Contador.Text = "Total Reg.: " + Mi_REG.Count;
                    this.Txt_Filtro.ReadOnly = false; this.Txt_Filtro.Text = string.Empty;
                    this.Lv_Clientes.Enabled = true; this.Lv_Clientes.Select();
                    this.Lv_Clientes.Items[0].Selected = true;
                ...
                else
                {
                    ...
                    this.Txt_Negocio.Text = string.Empty;
                    ...
                    this.Lv_Clientes.Enabled = false;
        ...
        private bool Validaciones()
        {
            ...
            if (string.IsNullOrEmpty(this.Txt_Cliente.Text.Trim()) == true)
            {
                this.ep_Error.SetError(this.Txt_Cliente, "Debe de Digitar el Nombre del Cliente...");
                yResult = false;
            }

            if (string.IsNullOrEmpty(this.Txt_Negocio.Text.Trim()) == true)
            {
                this.ep_Error.SetError(this.Txt_Negocio, "Debe de Digitar el Nombre del Negocio...");
                yResult = false;
            }

            if (string.IsNullOrEmpty(this.Txt_Direccion.Text.Trim()) == true)
            {
                this.ep_Error.SetError(this.Txt_Direccion, "Debe de Digitar la Direccion del Cliente o Negocio...");
                yResult = false;
            }
            
            if (string.IsNullOrEmpty(this.Txt_Telefono.Text.Trim()) == true)
            {
                this.ep_Error.SetError(this.Txt_Telefono, "Debe de Digitar el Telefono del Cliente o Negocio...");
                yResult = false;
            }
        ...
        private void Frm_Clientes_VisibleChanged(object sender, EventArgs e)
        {
        ...
        private void Cmd_Nuevo_Click(object sender, EventArgs e)
        {
            ...
            this.Txt_Cliente.ReadOnly = false;
            this.Txt_Negocio.ReadOnly = false;
        ...
        private void Cmd_Guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<cCLIENTES> Mis_Datos = new List<cCLIENTES>();
                cCLIENTES xDatos = new cCLIENTES();
                xDatos.ID = (this.Txt_Codigo.Text.Trim().Length == 0) ? "0" : double.Parse(this.Txt_Codigo.Text).ToString().Trim();
                xDatos.DESCRIPCION = this.Txt_Cliente.Text.Trim();
                xDatos.NEGOCIO = this.Txt_Negocio.Text; xDatos.DIRECCION = this.Txt_Direccion.Text;
                xDatos.TELEFONOS = this.Txt_Telefono.Text; xDatos.TIPO = (this.Cbo_Tipo.Text == "NATURAL") ? "1" : "2";
                ...
                Program.xMsg = string.Empty;
                Program.xMsg = Program.xMETODOS.Insertar_Actualizar_Clientes(Dt_DATOS);
                if (Program.xMsg.Length > 0)
                {
                    MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Program.Bool_Entrar = false; return;
        ...
        private void Txt_Filtro_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            try
            {
                if (e.KeyChar == (char)Keys.Enter)
                {
                    ...
                    Program.xMsg = string.Empty;
                    Mi_REG = Program.xMETODOS.Listar_Clientes(out Program.xMsg, this.Txt_Filtro.Text);
                    if (Program.xMsg.Length > 0)
                    {
                        MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    ...
                    {
                        this.Lbl_Contador.Text = "Total Reg.: " + Mi_REG.Count;
                        this.Lv_Clientes.Select(); this.Lv_Clientes.Items[0].Selected = true;
                    }
                    else
                    {
                        this.Txt_Codigo.Text = string.Empty;
        ...
        private void Txt_Cliente_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (this.Txt_Cliente.ReadOnly == true)
                {
                    e.Handled = true;
                    return;
                }
                this.Txt_Negocio.Select();
            }
        ...
        private void Lv_Clientes_ImeModeChanged(object sender, EventArgs e)
        {
            
        }

        private void Lv_Clientes_DoubleClick(object sender, EventArgs e)
        {
        ...
        private void Txt_Negocio_KeyPress(object sender, KeyPressEventArgs e)
        {
            // Valida los caracteres de entradas
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (this.Txt_Negocio.ReadOnly == true)
                {
                    e.Handled = true;
                    return;
                }
        ...
        private void Txt_Telefono_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (this.Txt_Telefono.ReadOnly == true)
                {
                    e.Handled = true;
                    return;
                }
        ...
        private void Lv_Clientes_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            if (this.Lv_Clientes.Items.Count > 0)
            {
                ...
                {
                    // Enviando datos
                    this.Txt_Codigo.Text = this.Lv_Clientes.SelectedItems[0].SubItems[0].Text;
                    this.Txt_Cliente.Text = this.Lv_Clientes.SelectedItems[0].SubItems[1].Text;
                    this.Txt_Negocio.Text = this.Lv_Clientes.SelectedItems[0].SubItems[2].Text;
                    this.Txt_Direccion.Text = this.Lv_Clientes.SelectedItems[0].SubItems[3].Text;
                    this.Txt_Telefono.Text = this.Lv_Clientes.SelectedItems[0].SubItems[4].Text;
                    this.Cbo_Tipo.Text = this.Lv_Clientes.SelectedItems[0].SubItems[5].Text;
                    this.Chk_Activo.Checked = (this.Lv_Clientes.SelectedItems[0].SubItems[6].Text == "SI") ? true : false;

                    // Para Devolverselo 
                    if (Program.Val_Ban == 1)
                    {
                        Frm_Factura.xID_CLIENTE = double.Parse(this.Txt_Codigo.Text).ToString();
                        Frm_Factura.xCOD_CLIENTE = this.Txt_Codigo.Text;
                        Frm_Factura.xNOMB_CLIENTE = this.Txt_Cliente.Text;
                        Frm_Factura.xTIPO_CLIENTE = this.Cbo_Tipo.Text;
                    }
                }
            }
        }
