...
namespace Capa_Diseño
{
    ...
    {
        // Para la Ventana de PROVEEDOR
        public static string xID_PROVEEDOR = string.Empty, xCOD_PROVEEDOR = string.Empty, xNOMB_PROVEEDOR = string.Empty;
        ...
        private static List<TBL_ENTRADAS_PRODUCTOS> Mi_REG_ENC = new List<TBL_ENTRADAS_PRODUCTOS>();

        private void Cmd_Nuevo_Click(object sender, EventArgs e)
        {
            this.Dtp_Fecha.Enabled = true;
            
            this.Txt_IdEntrada.Text = string.Empty;
            ...
            this.Txt_CodProveedor.Text = string.Empty;
            ...
            Mi_REG_DET = Program.xMETODOS.Listar_DET_Entradas_Productos(out Program.xMsg, "0");
            if (Program.xMsg.Length > 0)
            {
                MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
                Program.Bool_Entrar = false;
            ...
            xID_PROVEEDOR = string.Empty; xCOD_PROVEEDOR = string.Empty; xNOMB_PROVEEDOR = string.Empty;
            xID_PRODUCTOS = string.Empty; xCOD_PRODUCTOS = string.Empty; xDESC_PRODUCTOS = string.Empty;
            xID_UM = string.Empty; xDESC_UM = string.Empty;
        }
        ...
        private void Cmd_Cancelar_Click(object sender, EventArgs e)
        {
            ...
            Frm_Entrada_Load(sender, e);
        }

        private void Lv_Entradas_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
        ...
        private void Cmd_Anular_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Program.xMsg = string.Empty;
                Program.xMsg = Program.xMETODOS.Anular_Entradas_Productos(double.Parse(this.Txt_IdEntrada.Text).ToString());
                if (Program.xMsg.Length > 0)
                {
                    MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void Btn_AgregarDetalle_Click(object sender, EventArgs e)
        {
            ...
            double xNeto = 0.00;
            for (int I = 0; I <= (this.Lv_Entradas.Items.Count - 1); I++)
            { xNeto = xNeto + double.Parse(this.Lv_Entradas.Items[I].SubItems[7].Text); }
            this.Txt_Neto.Text = string.Format("{0:##0.00}", xNeto);
        ...
        private void Btn_Quitar_Click(object sender, EventArgs e)
        {
            ...
            if (Program.Bool_SELECCION == false)
        ...
        private bool Validar_Detalle()
        {
            ...
            if (string.IsNullOrEmpty(this.Txt_Cantidad.Text.Trim()) == true)
            {
        ...
        private void Cmd_Guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<cENTRADAS_PRODUCTOS> Mis_Datos = new List<cENTRADAS_PRODUCTOS>();
                cENTRADAS_PRODUCTOS xDatos = new cENTRADAS_PRODUCTOS();
                xDatos.ID = (this.Txt_IdEntrada.Text.Trim().Length == 0) ? "0" : double.Parse(this.Txt_IdEntrada.Text).ToString().Trim();
                xDatos.FECHA = string.Format("{0:yyyy-MM-dd}", this.Dtp_Fecha.Value); xDatos.ID_PROVEEDOR = xID_PROVEEDOR;
                ...
                List<cDETALLE_ENTRADAS_PRODUCTOS> Mis_Datos_DET = new List<cDETALLE_ENTRADAS_PRODUCTOS>();
                cDETALLE_ENTRADAS_PRODUCTOS xDatos_DET = new cDETALLE_ENTRADAS_PRODUCTOS();

                for (int I = 0; I <= (this.Lv_Entradas.Items.Count - 1); I++)
                {
                    xDatos_DET = new cDETALLE_ENTRADAS_PRODUCTOS();
                ...
                Program.xMsg = string.Empty;
                Program.xMsg = Program.xMETODOS.Insertar_Entradas_Productos(Dt_ENC, Dt_DET);
                if (Program.xMsg.Length > 0)
                {
                    MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Program.Bool_Entrar = false; return;
        ...
        {            
            try
            {
                ...
                Program.xMsg = string.Empty;
                List<TBL_ENTRADAS_PRODUCTOS> Mi_REG_ENC = Program.xMETODOS.Listar_ENC_Entradas_Productos(out Program.xMsg, this.Txt_Filtro.Text, string.Format("{0:yyyy-MM-dd}", this.Dtp_Desde.Value), string.Format("{0:yyyy-MM-dd}", this.Dtp_Hasta.Value));
                if (Program.xMsg.Length > 0)
                {
                    MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Program.Bool_Entrar = false;
        ...
        private static List<TBL_DETALLE_ENTRADAS_PRODUCTOS> Mi_REG_DET = new List<TBL_DETALLE_ENTRADAS_PRODUCTOS>();
        ...
        private void Lv_Busqueda_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            if (this.Lv_Busqueda.Items.Count > 0)
            {
                ...
                {
                    // Enviando datos
                    this.Txt_IdEntrada.Text = this.Lv_Busqueda.SelectedItems[0].SubItems[1].Text;
                    this.Dtp_Fecha.Value = DateTime.Parse(this.Lv_Busqueda.SelectedItems[0].SubItems[0].Text);
                    xID_PROVEEDOR = double.Parse(this.Lv_Busqueda.SelectedItems[0].SubItems[2].Text).ToString();
                    this.Txt_CodProveedor.Text = this.Lv_Busqueda.SelectedItems[0].SubItems[2].Text;
                    this.Txt_Proveedor.Text = this.Lv_Busqueda.SelectedItems[0].SubItems[3].Text;
                    this.Txt_Neto.Text = this.Lv_Busqueda.SelectedItems[0].SubItems[4].Text;
                    this.Chk_Anulado.Checked = (this.Lv_Busqueda.SelectedItems[0].SubItems[5].Text == "SI") ? true : false;
                    ...
                    //Recuperando Registros
                    Program.xMsg = string.Empty;
                    Mi_REG_DET = Program.xMETODOS.Listar_DET_Entradas_Productos(out Program.xMsg, double.Parse(this.Txt_IdEntrada.Text).ToString());
                    if (Program.xMsg.Length > 0)
                    {
                        MessageBox.Show(Program.xMsg, Program.SISTEMA, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Program.Bool_Entrar = false;
        ...
        private void Cmd_Imprimir_Click(object sender, EventArgs e)
        {
            ...
            Program.xReporte = 6; Program.xID_ENTRADA = double.Parse(this.Txt_IdEntrada.Text);

            Frm_Visualizador xForm = new Frm_Visualizador();
            xForm.ShowDialog();
        }

        private void Frm_Entrada_VisibleChanged(object sender, EventArgs e)
        {
            if (Program.Bool_Entrar == false)
            { Program.xAcceso = 0; this.Close(); }
        }

        private void Frm_Entrada_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        // Llenar Listado
        private void Llenar_Lista_Detalle(ref ListView xContenedor, List<TBL_DETALLE_ENTRADAS_PRODUCTOS> nDatos)
        {
            ...
            if (nDatos != null)
            {
                ...
                foreach (TBL_DETALLE_ENTRADAS_PRODUCTOS xREG in nDatos)
                {
        ...
        private void Llenar_Lista_Encabezado(ref ListView xContenedor, List<TBL_ENTRADAS_PRODUCTOS> nDatos)
        {
            // Limpiando el Listview
            xContenedor.Clear();
            xContenedor.Refresh();
            ...
            xContenedor.Columns.Add("PROVEEDOR", 400, HorizontalAlignment.Left);
            xContenedor.Columns.Add("TOTAL", 120, HorizontalAlignment.Right);
            xContenedor.Columns.Add("ANULADO", 100, HorizontalAlignment.Center);
            
            if (nDatos != null)
            {
                // Llenando el Listview
                foreach (TBL_ENTRADAS_PRODUCTOS xREG in nDatos)
                {
                    ListViewItem xFila = new ListViewItem(xREG.FECHA, 0);
                    xFila.SubItems.Add(xREG.ID);
                    xFila.SubItems.Add(xREG.ID_PROVEEDOR);
                    xFila.SubItems.Add(xREG.NOMBRE_PROVEEDOR);
                    xFila.SubItems.Add(xREG.TOTAL);
        ...
        {            
            ...
            try
            {
                ...
                this.Txt_IdEntrada.ReadOnly = true;
                ...
                this.Txt_CodProveedor.ReadOnly = true;
        ...
        {
            // Abriendo la Ventana de Proveedores
            xID_PROVEEDOR = string.Empty; xCOD_PROVEEDOR = string.Empty; xNOMB_PROVEEDOR = string.Empty;

            Program.Val_Ban = 1;
            Frm_Proveedores xForm = new Frm_Proveedores();
            xForm.ShowDialog();

            Program.Val_Ban = 0;

            if (xID_PROVEEDOR.Trim().Length == 0)
            {
                this.Txt_CodProveedor.Text = string.Empty;
                this.Txt_Proveedor.Text = string.Empty;
            ...
            // Asignando Datos
            this.Txt_CodProveedor.Text = xCOD_PROVEEDOR; this.Txt_Proveedor.Text = xNOMB_PROVEEDOR;
